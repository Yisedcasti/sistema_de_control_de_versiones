...
function response($code = 200, $status = "", $message = ""){
    ...
    $tareasDB =  new TareasDB();
        if (isset($_GET['id_cursos'])){
            $response = $tareasDB->verificarExistenciaById($_GET['id_cursos']);
            echo json_encode($response, JSON_PRETTY_PRINT);
        } else{
    ...
    function ProcesarCrearNoticia()
    {
        try{
            ...
            if(empty($objArr)){
                ...
                print "el dato es $obj->curso";
                $tareasDB = new TareasDB();
                $tareasDB->registrarDatos($obj->curso,$obj->grado_id_grado);
    ...
    function ProcesarEliminarNoticia()
    {
        if (isset($_GET['action']) && isset($_GET['id'])) {
            if ($_GET['action'] == 'delete') {
                $id_cursos = intval($_GET['id']);
                $tareasDB = new TareasDB();
                
                if ($tareasDB->verificarExistenciaById($id_cursos)) {
                    $respuesta = $tareasDB->eliminarNoticia($id_cursos);
                    if ($respuesta) {
